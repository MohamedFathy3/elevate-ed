...
export const AssignmentCard = ({ 
  ...
  useEffect(() => {
    console.log(`🎴 [AssignmentCard] Assignment ${assignmentIndex + 1}: "${assignment.title}"`);
  ...
  }, [assignment, assignmentIndex, isPassed, isFailed, isLocked, isHidden, isWaitingResult, isWaitingCorrection, showMessageOnly, studentPassedMessage, notSolved]);
  ...
  // ✅ إذا كان الطالب لم يحل الواجب بعد
  if (notSolved) {
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-bold text-gray-900 dark:text-white text-sm">
              {isRtl ? `الواجب ${assignmentIndex + 1}` : `Assignment ${assignmentIndex + 1}`}
            </h3>
      ...
      {/* ✅ إذا كان الطالب لم يحل الواجب */}
      {notSolved ? (
        <>
          <div className="flex flex-wrap items-center gap-3 text-xs text-gray-500 dark:text-gray-400 mb-3">
            <span>📝 {assignment.total_marks} درجة</span>
            <span>🎯 {assignment.total_must_pass_marks} درجة للنجاح</span>
            {assignment.duration_minutes && (
              <span>⏱️ {assignment.duration_minutes} دقيقة</span>
            )}
          </div>
          ...
          >
            ...
            {isRtl ? "ابدأ الواجب" : "Start Assignment"}
          </button>
        </>
        ...
        <>
          <div className="flex flex-wrap items-center gap-3 text-xs text-gray-500 dark:text-gray-400 mb-3">
            ...
            {!isWaitingResult && !isWaitingCorrection && isFailed && (
              <span className="text-red-600 dark:text-red-400 font-semibold">
                ❌ حصلت على {assignment.total || 0} من {assignment.total_must_pass_marks || 0}
              </span>
            )}
          ...
          {!isWaitingResult && !isWaitingCorrection && !isPassed && !isFailed && !isLocked && (
            <button
              onClick={onStart}
              className="w-full py-2 rounded-xl font-semibold flex items-center justify-center gap-2 text-sm bg-gradient-to-r from-blue-500 to-indigo-500 hover:from-blue-600 hover:to-indigo-600 text-white shadow-lg shadow-blue-500/25 transition-all"
            >
          ...
          {!isWaitingResult && !isWaitingCorrection && isLocked && !isPassed && !isFailed && (
