...
// ✅ Component للصورة المحسنة - تظهر فوراً
const OptimizedStageImage = ({ 
  src, 
  alt, 
  className = "",
  priority = false,
  ...
  return (
    <div className="relative w-full h-full overflow-hidden bg-gray-200 dark:bg-gray-800">
      {/* Skeleton - يختفي فوراً لما تتحمل الصورة */}
      {!isLoaded && !error && (
...
export const Stage = () => {
  ...
  // ✅ Nature Theme - من غير Animations
  if (isNature) {
    const stageColors = [
    ...
    return (
      <section id="stages" className="relative overflow-hidden py-24 md:py-32 bg-gradient-to-br from-emerald-50 via-white to-teal-50 dark:from-gray-950 dark:via-emerald-950/10 dark:to-gray-950">
        {/* Background - بدون حركات */}
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute top-20 left-10 w-72 h-72 rounded-full bg-emerald-200/30 dark:bg-emerald-900/20 blur-3xl" />
          <div className="absolute bottom-20 right-10 w-80 h-80 rounded-full bg-teal-200/20 dark:bg-teal-900/20 blur-3xl" />
        </div>
        ...
        <div className="container-tight relative z-10">
          ...
          <div className="text-center max-w-3xl mx-auto mb-16">
            ...
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold text-emerald-900 dark:text-emerald-100 mb-4">
              {lang === "ar" ? "السنوات الدراسية" : "Academic Years"}
            </h1>
            <p className="text-emerald-700/70 dark:text-emerald-300/70 text-lg">
              {lang === "ar"
                ...
                >
                  <div
                    className={`relative rounded-2xl overflow-hidden bg-white dark:bg-gray-900 shadow-lg transition-all duration-300 h-full flex flex-col
                      ${isDisabled ? 'opacity-80 grayscale-[0.1]' : 'hover:shadow-2xl dark:hover:shadow-emerald-900/30 hover:-translate-y-2'}`}
                  >
                    {/* Image - مع تحسين فوري */}
    ...
    <section id="stages" className="relative overflow-hidden py-28 md:py-36 bg-white dark:bg-gray-950">
      <div className="container-tight relative z-10">
        ...
        <div className="mx-auto max-w-4xl text-center">
          ...
          <p className="mx-auto mt-8 max-w-2xl text-lg leading-9 text-gray-600 dark:text-gray-400">
            {lang === "ar"
        ...
        <div className="mt-16 grid gap-5 md:grid-cols-3">
          {[
            ...
            return (
              <div
                key={i}
                className="group rounded-[32px] border border-gray-200 dark:border-gray-800 p-7 backdrop-blur-2xl bg-white/60 dark:bg-gray-900/60 transition-all hover:-translate-y-2"
              >
            ...
            {visibleStages.map((s: any, i: number) => {
              ...
              return (
                <SwiperSlide key={s.id}>
                  ...
                  >
                    <div className={`relative overflow-hidden rounded-[36px] border border-gray-200 dark:border-gray-800 backdrop-blur-2xl bg-white/60 dark:bg-gray-900/60 transition-all duration-500 ${!isDisabled ? 'hover:border-primary/30 dark:hover:border-primary/50 hover:shadow-2xl hover:-translate-y-3' : ''}`}>
                      {isDisabled && (
                      ...
                      <div className="relative h-[320px] overflow-hidden">
                        ...
                        <div className="absolute bottom-5 left-5 rounded-2xl border border-white/10 bg-white/10 dark:bg-black/30 px-4 py-3 backdrop-blur-2xl">
    ...
    <section className={`py-32 ${bgClass}`}>
      <div className="container-tight">
        <div className="mx-auto mb-20 max-w-3xl text-center">
          <div className={`mx-auto h-10 w-48 animate-pulse rounded-full ${pulseClass}`} />
          <div className={`mx-auto mt-6 h-20 w-full animate-pulse rounded-3xl ${pulseClass}`} />
          <div className={`mx-auto mt-6 h-8 w-2/3 animate-pulse rounded-2xl ${pulseClass}`} />
        </div>
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
          {[1, 2, 3, 4, 5, 6].map((i) => (
            <div key={i} className={`overflow-hidden rounded-3xl border ${borderClass} bg-white/80 dark:bg-gray-900/40`}>
              <div className={`h-64 animate-pulse ${pulseClass}`} />
              <div className="p-6">
                <div className={`h-8 w-2/3 animate-pulse rounded-2xl ${pulseClass}`} />
                <div className={`mt-4 h-16 animate-pulse rounded-2xl ${pulseClass}`} />
                <div className="mt-5 flex gap-3">
                  <div className={`h-8 w-20 animate-pulse rounded-full ${pulseClass}`} />
                  <div className={`h-8 w-20 animate-pulse rounded-full ${pulseClass}`} />
                </div>
              </div>
            </div>
